...
const DEFAULT_SERVO_PROFILE = "sms_sts";
const DETECTION_SERVO_PROFILES = ["sms_sts", "scscl"];
...
function signedMagnitude(value, signBit) {
  const rounded = Math.round(value);
  let servoValue = Math.abs(rounded) & ((1 << signBit) - 1);
  if (rounded < 0) {
    servoValue |= 1 << signBit;
  }
  return servoValue;
}

function normalizeServoIds(servoIds) {
  const ids = Array.isArray(servoIds) ? servoIds : [servoIds];
  return ids
    .map((id) => parseInt(id, 10))
    .filter((id) => !Number.isNaN(id) && id > 0 && id < BROADCAST_ID);
}

function isInServoRange(value, profile) {
  return Number.isFinite(value) && value >= 0 && value <= profile.maxPosition;
}
...
export class ScsServoSDK {
  constructor(options = {}) {
    ...
    this.servoProfile = cloneProfile(
      getServoProfile(options.servoModel || options.servoType || DEFAULT_SERVO_PROFILE)
    );
    this.lastDetection = null;
  }

  setServoProfile(profileId = DEFAULT_SERVO_PROFILE) {
    const normalized = normalizeServoProfileId(profileId);
    if (normalized === "auto") {
      throw new Error("Use detectServoProfile() or connect({ servoModel: 'auto' }) for auto detection.");
    }
    this.servoProfile = cloneProfile(getServoProfile(normalized));
    if (this.packetHandler) {
      this.packetHandler = new PacketHandler(this.servoProfile.protocolEnd);
    }
    return this.getServoProfile();
  }

  getServoProfile() {
    return cloneProfile(this.servoProfile);
  }

  getPositionRange() {
    return { min: 0, max: this.servoProfile.maxPosition };
  }

  getAddress(name) {
    const address = this.servoProfile.addresses[name];
    if (address === null || address === undefined) {
      throw new Error(
        `${this.servoProfile.label} servos do not support address "${name}".`
      );
    }
    return address;
  }
  ...
  validatePosition(position, context) {
    if (position < 0 || position > this.servoProfile.maxPosition) {
      throw new Error(
        `Invalid position value ${position}${context ? ` for ${context}` : ""}. Must be between 0 and ${this.servoProfile.maxPosition} for ${this.servoProfile.label} servos.`
      );
    }
  }
  ...
  async connect(options = {}) {
    ...
    const requestedProfileId =
      servoModel ||
      servoType ||
      (typeof protocolEnd === "number"
        ? protocolEnd === 1
          ? "scscl"
          : "sms_sts"
        : this.servoProfile.id);
    const normalizedProfileId = normalizeServoProfileId(requestedProfileId);
    if (normalizedProfileId !== "auto") {
      this.servoProfile = cloneProfile(getServoProfile(normalizedProfileId));
    } else {
      this.servoProfile = cloneProfile(getServoProfile(DEFAULT_SERVO_PROFILE));
    }
    try {
      ...
      this.packetHandler = new PacketHandler(this.servoProfile.protocolEnd);
      if (normalizedProfileId === "auto") {
        const idsToDetect = detectionServoIds || detectServoIds || [1];
        this.lastDetection = await this.detectServoProfile(idsToDetect);
      }
      console.log(
        `Connected to serial port at ${baudRate} baud, servo profile: ${this.servoProfile.label} (protocol end: ${this.servoProfile.protocolEnd}).`
      );
      return true;
    } catch (err) {
  ...
  async readModelNumber(servoId) {
    this.checkConnection();
    const [modelNumber, result, error] = await this.packetHandler.read2ByteTxRx(
      this.portHandler,
      servoId,
      3
    );
    if (result !== COMM_SUCCESS) {
      throw new Error(
        `Error reading model number from servo ${servoId}: ${this.packetHandler.getTxRxResult(
          result
        )}, Error code: ${error}`
      );
    }
  ...
  async probeServoProfile(profileId, servoId) {
    this.setServoProfile(profileId);
    const profile = this.getServoProfile();
    const probe = {
      servoId,
      profileId: profile.id,
      label: profile.label,
      protocolEnd: profile.protocolEnd,
      score: 0,
      errors: [],
    };

    try {
      probe.modelNumber = await this.readModelNumber(servoId);
      const expectedModels = profile.expectedModelNumbers || [];
      probe.exactModelMatch = expectedModels.includes(probe.modelNumber);
      if (probe.exactModelMatch) {
        probe.score += 6;
      } else if (expectedModels.length > 0 && probe.modelNumber > 0) {
        probe.score -= 2;
      } else if (probe.modelNumber > 0 && probe.modelNumber <= 10000) {
        probe.score += 1;
      } else if (probe.modelNumber > 0) {
        probe.score -= 1;
      }
    } catch (err) {
      probe.errors.push(`model: ${err.message}`);
    }

    try {
      probe.position = await this.readPosition(servoId);
      probe.positionInRange = isInServoRange(probe.position, profile);
      if (probe.positionInRange) {
        probe.score += 3;
      } else {
        probe.score -= 4;
        if (probe.position > 4095) {
          probe.score -= 2;
        }
      }
    } catch (err) {
      probe.errors.push(`position: ${err.message}`);
    }

    try {
      probe.maxPositionLimit = await this.readMaxPosLimit(servoId);
      probe.maxPositionLimitInRange = isInServoRange(
        probe.maxPositionLimit,
        profile
      );
      if (probe.maxPositionLimitInRange) {
        probe.score += 4;
      } else {
        probe.score -= 4;
        if (probe.maxPositionLimit > 4095) {
          probe.score -= 2;
        }
      }
    } catch (err) {
      probe.errors.push(`max limit: ${err.message}`);
    }

    return probe;
  }

  async detectServoProfile(servoIds = [1]) {
    this.checkConnection();
    const ids = normalizeServoIds(servoIds);
    if (ids.length === 0) {
      throw new Error("Auto detection needs at least one valid servo ID.");
    }

    const previousProfileId = this.servoProfile.id;
    const probes = [];
    for (const id of ids) {
      for (const profileId of DETECTION_SERVO_PROFILES) {
        try {
          probes.push(await this.probeServoProfile(profileId, id));
        } catch (err) {
          probes.push({
            servoId: id,
            profileId,
    ...
    probes.sort((a, b) => {
      ...
      if (Number(b.exactModelMatch) !== Number(a.exactModelMatch)) {
        return Number(b.exactModelMatch) - Number(a.exactModelMatch);
      }
      return (a.errors?.length || 0) - (b.errors?.length || 0);
    });
    const best = probes[0];
    if (!best || best.score <= 0) {
      this.setServoProfile(previousProfileId);
      throw new Error(
        `Unable to detect servo profile from IDs ${ids.join(", ")}. Select STS/SMS or SCS/SCSCL/SCS225 manually.`
      );
    }

    this.setServoProfile(best.profileId);
    this.lastDetection = {
      ...best,
      probes,
      selectedProfile: this.getServoProfile(),
    };
  ...
  async readPosition(servoId) {
    ...
    try {
      const [position, result, error] = await this.packetHandler.read2ByteTxRx(
        this.portHandler,
        servoId,
        this.getAddress("presentPosition")
      );
      if (result !== COMM_SUCCESS) {
        throw new Error(
  ...
  async writePosition(servoId, position) {
    this.checkConnection();
    try {
      this.validatePosition(position, `servo ${servoId}`);
      const targetPosition = Math.round(position);
      let result;
      let error;
      if (this.servoProfile.positionWriteLength === 6) {
        const time = 0;
        const speed = 0;
        const data = [
          SCS_LOBYTE(targetPosition),
          SCS_HIBYTE(targetPosition),
          SCS_LOBYTE(time),
          SCS_HIBYTE(time),
          SCS_LOBYTE(speed),
          SCS_HIBYTE(speed),
        ];
        [result, error] = await this.packetHandler.writeTxRx(
          this.portHandler,
          servoId,
          this.getAddress("goalPosition"),
          data.length,
          data
        );
      } else {
        [result, error] = await this.packetHandler.write2ByteTxRx(
          this.portHandler,
          servoId,
          this.getAddress("goalPosition"),
          targetPosition
  ...
  async writeAcceleration(servoId, acceleration) {
    this.checkConnection();
    if (!this.servoProfile.supportsAcceleration) {
      throw new Error(`${this.servoProfile.label} servos do not support acceleration writes.`);
    }
    try {
      ...
      const [result, error] = await this.packetHandler.write1ByteTxRx(
        this.portHandler,
        servoId,
        this.getAddress("goalAcceleration"),
        clampedAcceleration
      );
      if (result !== COMM_SUCCESS) {
        throw new Error(
  ...
  async setWheelMode(servoId) {
    ...
    try {
      ...
      let resMode;
      let errMode;
      if (this.servoProfile.modeControl === "angleLimits") {
        [resMode, errMode] = await this.packetHandler.writeTxRx(
          this.portHandler,
          servoId,
          this.getAddress("minPosLimit"),
          4,
          [0, 0, 0, 0]
        );
      } else {
        [resMode, errMode] = await this.packetHandler.write1ByteTxRx(
          this.portHandler,
          servoId,
          this.getAddress("mode"),
          1
  ...
  async setPositionMode(servoId) {
    ...
    try {
      ...
      let errMode;
      if (this.servoProfile.modeControl === "angleLimits") {
        const max = this.servoProfile.maxPosition;
        [resMode, errMode] = await this.packetHandler.writeTxRx(
          this.portHandler,
          servoId,
          this.getAddress("minPosLimit"),
          4,
          [0, 0, SCS_LOBYTE(max), SCS_HIBYTE(max)]
        );
      } else {
        [resMode, errMode] = await this.packetHandler.write1ByteTxRx(
          this.portHandler,
  ...
  async readPosCorrection(servoId) {
    this.checkConnection();
    if (!this.servoProfile.supportsPositionCorrection) {
      throw new Error(`${this.servoProfile.label} servos do not support position correction.`);
    }
    try {
      const [rawValue, result, error] = await this.packetHandler.read2ByteTxRx(
        this.portHandler,
        servoId,
        this.getAddress("posCorrection")
      );
      if (result !== COMM_SUCCESS) {
        throw new Error(
  ...
  async writeWheelSpeed(servoId, speed) {
    this.checkConnection();
    try {
      const maxSpeed = this.servoProfile.wheelSpeedMax;
      const clampedSpeed = Math.max(-maxSpeed, Math.min(maxSpeed, Math.round(speed)));
      const speedValue = signedMagnitude(
        clampedSpeed,
        this.servoProfile.wheelSpeedSignBit
      );
      const [result, error] = await this.packetHandler.write2ByteTxRx(
        this.portHandler,
        servoId,
        this.servoProfile.modeControl === "angleLimits"
          ? this.getAddress("goalTime")
          : this.getAddress("goalSpeed"),
        speedValue
      );
      if (result !== COMM_SUCCESS) {
        throw new Error(
  ...
  async syncWriteWheelSpeed(servoSpeeds) {
    ...
    for (const [idStr, speed] of entries) {
      ...
      if (speed < -maxSpeed || speed > maxSpeed) {
        throw new Error(
          `Invalid speed value ${speed} for servo ${servoId} in syncWriteWheelSpeed. Must be between ${-maxSpeed} and ${maxSpeed}.`
        );
      }
      const clampedSpeed = Math.max(-maxSpeed, Math.min(maxSpeed, Math.round(speed)));
      const speedValue = signedMagnitude(
  ...
  async syncReadPositions(servoIds) {
    ...
    const startAddress = this.getAddress("presentPosition");
  ...
  async syncWritePositions(servoPositions) {
    ...
    const groupSyncWrite = new GroupSyncWrite(
      ...
      this.packetHandler,
      this.getAddress("goalPosition"),
      this.servoProfile.syncPositionWriteLength
    );
    let paramAdded = false;
    const entries =
    ...
    for (const [idStr, position] of entries) {
      ...
      if (position < 0 || position > this.servoProfile.maxPosition) {
        throw new Error(
          `Invalid position value ${position} for servo ${servoId} in syncWritePositions. Must be between 0 and ${this.servoProfile.maxPosition}.`
        );
      }
      const targetPosition = Math.round(position);
      this.validatePosition(targetPosition, `servo ${servoId} in syncWritePositions`);
      const data =
        this.servoProfile.syncPositionWriteLength === 6
          ? [
              SCS_LOBYTE(targetPosition),
              SCS_HIBYTE(targetPosition),
              0,
          ...
          : [SCS_LOBYTE(targetPosition), SCS_HIBYTE(targetPosition)];
      if (groupSyncWrite.addParam(servoId, data)) {
        paramAdded = true;
      } else {
        console.warn(
  ...
  async writeMaxPosLimit(servoId, limit) {
    ...
    let unlocked = false;
    try {
      if (limit < 0 || limit > this.servoProfile.maxPosition) {
        throw new Error(
          `Invalid limit value ${limit} for servo ${servoId}. Must be between 0 and ${this.servoProfile.maxPosition}.`
        );
      }
