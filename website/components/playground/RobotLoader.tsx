...
export default function RobotLoader({ robotName }: RobotLoaderProps) {
  ...
  useEffect(() => {
    const keyboardStored = getPanelStateFromLocalStorage(
      "keyboardControl",
      robotName
    );
    setShowControlPanel(
      keyboardStored !== null ? keyboardStored : window.innerWidth >= 900
    );
    setShowLeaderControl(
      getPanelStateFromLocalStorage("leaderControl", robotName) ?? false
    );
    setShowChatControl(
      getPanelStateFromLocalStorage("chatControl", robotName) ?? false
    );
    setShowRecordControl(
      getPanelStateFromLocalStorage("recordControl", robotName) ?? false
    );
  }, [robotName]);
  ...
  return (
    <>
      ...
      <ControlPanel
        ...
        onServoSelectionChange={setServoSelection}
        maxServoPosition={maxServoPosition}
        availableServoIds={availableServoIds}
