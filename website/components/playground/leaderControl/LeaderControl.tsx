...
type LeaderControlProps = {
  ...
  jointDetails: LeaderJoint[];
  onSync: (leaderAngles: LeaderAngle[]) => void;
  show?: boolean;
  onHide?: () => void;
};
...
const LeaderControl = ({
  ...
  useEffect(() => {
    ...
    let timer = setInterval(async () => {
      ...
      const leaderAngles = revoluteJoints.map((j) => ({
        servoId: j.servoId,
        angle: servoPositionToAngle(positions.get(j.servoId) ?? 0, positionMax),
      }));
  ...
  }, [isConnected, revoluteJoints, getPositions, onSync, positionMax]);
